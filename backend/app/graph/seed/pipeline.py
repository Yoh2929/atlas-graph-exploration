...
class SeedPipeline:
    ...
    def build(self) -> SeedSnapshot:
        ...
        timer = monotonic()
        unlimited = self.settings.max_nodes == 0
        if unlimited:
            discovery_budget = None
            notable_people_budget = None
            wikipedia_budget = None
            print(
                "[seed 1/4] Decouverte Wikipedia/Wikidata sans plafond global "
                f"(profondeur {self.settings.wikipedia_depth})",
                flush=True,
            )
        else:
        ...
        if discovery_budget is not None:
            discovered = discovered[:discovery_budget]
