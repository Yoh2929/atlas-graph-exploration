...
export default function App() {
  const { graph, loading: graphLoading, error } = useGraph(); const auth = useAuth(); const favorites = useFavoriteCollection(!!auth.user);
  ...
  return <div className="relative h-screen w-screen overflow-hidden bg-bg text-ink">
    <header className="pointer-events-none absolute inset-x-0 top-0 z-[60] flex h-[76px] items-center gap-1 px-2 sm:gap-3 sm:px-5">
      ...
      <div className="pointer-events-auto min-w-0 flex-1 sm:mx-auto sm:max-w-2xl"><SearchBar graph={graph} onSelectNode={handleSelectNode} onLensChange={(ids,label) => { setFocusIds(ids); setLensLabel(label); }} /></div>
    ...
    <main className="absolute inset-0">
      ...
      {!error && graphLoading && <div className="absolute inset-0 z-10 grid place-items-center bg-bg"><div className="text-center"><div className="mx-auto h-12 w-12 animate-spin rounded-full border border-white/10 border-t-category-algorithm"/><p className="mt-5 font-display text-xl">Ouverture du champ de connaissance…</p><p className="mt-1 text-xs text-ink-dim">Tous les nœuds, aucune coupe arbitraire</p></div></div>}
      {graph && <><GraphCanvas3D data={graph} selectedId={selectedId} focusIds={focusIds} favoriteIds={favorites.ids} onSelectNode={handleSelectNode}/><GraphControls lensLabel={lensLabel} onClearLens={clearLens} onGenerate={surprise}/>{!selectedId&&!lensLabel&&<WelcomePanel onSearch={() => document.getElementById("atlas-search")?.focus()} onDrift={surprise}/>}</>}
